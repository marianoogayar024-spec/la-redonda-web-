...
(() => {
  ...
  function createExternalModules(onResolved) {
    ...
    function ensureBabel() {
      ...
      babelLoading = new Promise((res, rej) => {
        ...
        const pre = window.__resources && window.__resources.babelStandalone;
        if (pre) { s.src = pre; } else { s.src = BABEL_URL; s.integrity = BABEL_SRI; }
    ...
    function load(kind, url, after) {
      ...
      const p = ready.then(() => {
        const preMap = window.__resources || {};
        const localUrl = /image-slot\.js$/.test(url) ? preMap.imageSlotJs : null;
  ...
  function loadScript(src, integrity) {
    return new Promise((resolve2, reject) => {
      ...
      const s = document.createElement("script");
      const preMap = window.__resources || {};
      const local = src === REACT_URL ? preMap.reactUmd : src === REACT_DOM_URL ? preMap.reactDomUmd : null;
      if (local) { s.src = local; } else { s.src = src; s.integrity = integrity; }
      s.crossOrigin = "anonymous";
      s.async = false;
      s.onload = () => resolve2();
